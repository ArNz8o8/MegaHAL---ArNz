 ...
 *
 * Additions and changes by ArNz|8o8
 *
 * Ver 4.1: March 2016
 * - Removed mentioning the nick when replying to someone
 * - tried to remove the errors when compiling
 *
...
char *megahal_start(Function * global_funcs)
{
	...
	putlog(LOG_MISC, "*", "MegaHAL v%s by ArNz|8o8 loaded.", VER);
...
static void megahal_report(int idx, int details)
{
	Context;
	if (details) {
		dprintf(idx, "     by Jason Hutchens, ^Baron^ and ArNz|8o8\n");
...
static int pub_megahal(char *nick, char *host, char *hand, char *channel, char *text)
{
	...
	if (chan != NULL) {
		sprintf(prefix, "PRIVMSG %s :", channel, nick);
...
static int dcc_megaver(struct userrec *u, int idx, char *text)
{
	Context;
	if (!floodcheck()) return 0;
	dprintf(idx, "MegaHAL module v%s by Jason Hutchens, ^Baron^ and ArNz|8o8\n", VER);
	return 0;
}
		    ...
		    char *channel, char *text)
{
	Context;
	if (!floodcheck()) return 0;
	dprintf(DP_HELP, "PRIVMSG %s :MegaHAL module v%s by Jason Hutchens, ^Baron^ and ArNz|8o8\n", channel, VER);
	return 0;
}
